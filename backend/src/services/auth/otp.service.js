...
class OTPService {
  ...
  async findOrCreateUser(mobileNumber) {
    ...
    if (!user) {
      ...
      // [MOBILE USER FIX] - Ensure mobileNumber is set for existing users
      // Also update name if it was auto-generated from mobile number
      const last4Digits = normalizedNumber.slice(-4);
      const isAutoGeneratedName = user.name && (
        user.name === `User ${last4Digits}` ||
        user.name.match(new RegExp(`^User \\d{4}$`))
      );

      // Update mobileNumber if not set
      if (!user.mobileNumber) {
        user.mobileNumber = normalizedNumber;
        logger.info('[MOBILE USER FIX] Updated mobileNumber for existing user', {
          userId: user._id,
